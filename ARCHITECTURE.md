...
- The GitHub APK release workflow (`.github/workflows/release.yml`) builds only that APK. The GitHub build type uses `applicationIdSuffix = ".github"` and `versionNameSuffix = "-github"`, so it installs beside the Play build as `com.hermeswebui.android.github` and reports a channel-specific version such as `0.1.7-github`.
- Manual GitHub APK runs create or update release `v<versionName>` from the checked-out commit; tag-triggered releases require the tag to match the Gradle Android `versionName` exactly, such as `v0.1.7`, before upload.
